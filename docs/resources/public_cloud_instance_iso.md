...
- `instance_id` (String) Instance's ID. This value cannot be changed.

### Optional

- `desired_iso_id` (String) The desired ISO ID. Removing this will detach the current ISO from the instance. Changing it will cause the current ISO to be detached and a new one to be attached to the instance.

### Read-Only

- `iso_id` (String) The ISO ID. Detaching/attaching iso_ids is an asynchronous operation. This attribute shows the current iso_id while `desired_iso_id` has the desired iso_id state.
- `name` (String)
