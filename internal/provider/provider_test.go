...
func TestAccPublicCloudInstanceIsoResource(t *testing.T) {
			...
			Steps: []resource.TestStep{
				// Create and Read testing
				{
					...
					resource "leaseweb_public_cloud_instance_iso" "test" {
						desired_iso_id = "ACRONIS_BOOT_MEDIA"
						instance_id = "ace712e9-a166-47f1-9065-4af0f7e7fce1"
					}`,
					Check: resource.ComposeAggregateTestCheckFunc(
						resource.TestCheckResourceAttr(
	...
	t.Run("importing sets desired_iso_id to iso_id", func(t *testing.T) {
		resource.Test(t, resource.TestCase{
			ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
			Steps: []resource.TestStep{
				{
	...
	t.Run(
		"updating to a non existent iso_id returns an error",
		func(t *testing.T) {
			resource.Test(t, resource.TestCase{
				ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
				Steps: []resource.TestStep{
					...
					},
					{
						Config: providerConfig + `
					resource "leaseweb_public_cloud_instance_iso" "test" {
						desired_iso_id = "tralala"
						instance_id = "ace712e9-a166-47f1-9065-4af0f7e7fce1"
					}`,
						ExpectError: regexp.MustCompile(
							`Attribute iso_id value must be one of`,
						),
					},
				},
			})
	...
	t.Run(
		"creating a resource with a non existent iso_id returns an error",
		func(t *testing.T) {
			resource.Test(t, resource.TestCase{
				ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
				Steps: []resource.TestStep{
	...
	t.Run("updating instance_id triggers an error", func(t *testing.T) {
		resource.Test(t, resource.TestCase{
			ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
			Steps: []resource.TestStep{
				{
				...
				},
				{
					...
					ExpectError: regexp.MustCompile(
						`Attribute instance_id value cannot be changed`,
					),
				},
			},
		})
