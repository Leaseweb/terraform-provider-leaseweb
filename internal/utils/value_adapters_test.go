...
func TestAdaptNullableStringEnumToStringValue(t *testing.T) {
	value := publicCloud.REGIONNAME_EU_WEST_3

	type args struct {
		value *publicCloud.RegionName
	}
	tests := []struct {
		name string
		args args
		...
		},
		{
			name: "value has been set",
			args: args{value: &value},
			want: basetypes.NewStringValue("eu-west-3"),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equalf(
				t,
				tt.want,
				AdaptNullableStringEnumToStringValue(tt.args.value),
				"AdaptNullableStringEnumToStringValue(%v)",
				tt.args.value,
			)
		})
	}
}
func ExampleAdaptNullableStringEnumToStringValue() {
	nullableStringEnum := publicCloud.REGIONNAME_EU_WEST_3
	value := AdaptNullableStringEnumToStringValue(&nullableStringEnum)
